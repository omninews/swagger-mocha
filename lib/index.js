...
SwaggerMocha.prototype.getResults = function(callback) {
  ...
  var testedPaths = Object.keys(paths).filter(function (path) {
    ...
    return {
      requestUrl: self.setPathParams(path, paths[path].get.parameters),
      path: path,
      status: status,
      method: method
    };
...
SwaggerMocha.prototype.generateValidRequest = function(data, spec) {
  ...
  return this.request[data.method || 'get'](data.requestUrl);
};
...
SwaggerMocha.prototype.testResults = function(callback) {
  ...
  this.results.forEach(function (test) {
    var schema = self.swagger.paths[test.path][test.method].responses[test.status].schema;

    describe(test.method.toUpperCase() + ' ' + test.requestUrl, function () {
      var assertion = tv4.validateMultiple(test.result, schema, false, true);

      if(!assertion.valid) {
        assertion.errors.forEach(function (err) {
          it('parameter: ' + err.dataPath, function () {
            throw err;
          });
        });
      }
